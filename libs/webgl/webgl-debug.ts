...
export module WebGLDebugUtils {
   ...
   */
  export function glFunctionArgToString(functionName: string, argumentIndex: number, value: number): string {
   ...
   */
  export function makeDebugContext(ctx: WebGLRenderingContext, opt_onErrorFunc?): WebGLRenderingContext {
    init(ctx);
    ...
    const makeErrorWrapper = (ctx: WebGLRenderingContext, functionName: string) => {
      return function() {
    ...
    const wrapper: WebGLRenderingContext = {} as WebGL2RenderingContext;
    for (const propertyName in ctx) {
  ...
  export function resetToInitialState(ctx: WebGLRenderingContext) {
  ...
  export function makeLostContextSimulatingContext(ctx: WebGLRenderingContext): IWrapper {
    const wrapper_: IWrapper = {};
    let contextId_ = 1;
...
interface IWrapper {
  ...
  registerOnContextLostListener?: (listener: unknown) => void;
  registerOnContextRestoredListener?: (listener: unknown) => void;
}